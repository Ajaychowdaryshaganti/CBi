...
echo "</style>";
    ...
    echo "<th>Part No</th>";
    echo "<th>Refill Qty</th>";
    echo "<th>Max</th>";
    echo "<th>Min</th>";
    echo "<th>Re-Order Qty</th>";
    echo "<th>Purchase Price</th>";
    echo "<th>Price For No of units</th>";
    echo "<th>Price Per Unit</th>";
    echo "<th>BF16 Back</th>";
    echo "<th>3rd Stock</th>";
    echo "<th>Quantity</th>";
    echo "<th>Total Value</th>";
    echo "<th>Last Updated</th>";
    echo "<th>Category</th>";
    echo "<th>Limit</th>";
    echo "<th>3rd Stock Available</th>";
    echo "</tr>";
    echo "<tr>";
    ...
    if ($row) {
        ...
        echo "<td contenteditable='true' data-column='RefillQty'>" . $row['RefillQty'] . "</td>";
        ...
        echo "<td contenteditable='true' data-column='BF16Back'>" . $row['BF16Back'] . "</td>";
        echo "<td contenteditable='true' data-column='3rdStock'>" . $row['3rdStock'] . "</td>";
        echo "<td contenteditable='false' data-column='Quantity'>" . $row['Quantity'] . "</td>";
		...
		echo "<td contenteditable='true' data-column='special'>";
		echo "<label><input type='radio' name='row[special]' value='0' " . (($row && $row['special'] == 0) ? 'checked' : '') . "> Yes</label>";
		echo "<label><input type='radio' name='row[special]' value='1' " . (($row && $row['special'] == 1) ? 'checked' : '') . "> No</label>";
		echo "</td>";
		
		
    } else {
        echo "<td colspan=\"18\">No data found.</td>";
    }
    echo "</tr>";
    echo "</table>";
    ...
    echo "<input type='hidden' name='row[RefillQty]' value='" . ($row ? $row['RefillQty'] : "") . "'>";
    ...
    echo "<input type='hidden' name='row[BF16Back]' value='" . ($row ? $row['BF16Back'] : "") . "'>";
    echo "<input type='hidden' name='row[3rdStock]' value='" . ($row ? $row['3rdStock'] : "") . "'>";
    ...
    echo "<input type='hidden' name='row[special]' value='" . ($row ? $row['special'] : 0) . "'>";
    echo "</form><br><br><div id=\"message\"></div></center>";
    echo "<div id=\"newbin\" class=\"form-cube2\">";
    echo "<form method=\"post\" id=\"myForm\" action=\"kanban-edit2.php\"><h3> Update Other bin </h3><br>";
    ...
    echo "<h2>" . $msg . "</h2><br>";
    echo "<form method=\"post\" id=\"myForm\" action=\"kanban-edit2.php\">";
    echo "<label> Bin Location: </label>";
    echo "<div class=\"input-field\" id=\"idFld1\">";
    echo "<input type=\"text\" id=\"binlocation\" length=\"5\" name=\"binlocation\" required>";
    echo "</div>";
...
?>
 ...
 <script>
     window.onload = function() {
        act('tab2'); // Apply the initial filter
...
$(document).ready(function() {
  $('td[contenteditable="true"]').blur(function() {
    ...
    // Check if the column is "special"
    if (column === 'special') {
      // Map the value to 0 or 1 based on the selected option
      value = $('input[name="row[special]"]:checked').val();
    }
