...
try {
    ...
    $response = "Something went wrong! Please try again. If the issue persists, please contact the administrator1.";
}
...
if ($flag) {
    try {
        ...
        $response = "Something wrong! Please try again. If the issue persists, please contact the administrator2.";
    }
}
...
if ($flag) {
    try {
        ...
        if ($conn) { 
            // Prepare the SQL statement with placeholders
            $updateQuery = "UPDATE reorderhmtp SET Received = Received + ? , BackOrder = ?,Status=?, lastupdated = ? WHERE PartNo = ?";
            $statement = mysqli_prepare($conn, $updateQuery);
        ...
        $response = "Something wrong! Please try again. If the issue persists, please contact the administrator3.";
    }
}

if ($newbackorder<=0 && $flag) {
    try {
        if ($conn) {
            // Prepare the SQL statement with placeholders
