...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($required > 0) {
        ...
        $selectQuery = "SELECT * FROM reordercables WHERE PartNo = ?";
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            $updateQuery = "UPDATE reordercables
            SET Required = ?,
            ...
            // Insert a new record
            $insertQuery = "INSERT INTO reordercables (BinLocation, PartName,Length, PartNo, Category, Required, BackOrder, PurchaseOrder, Comments)
            SELECT BinLocation, PartName,Length, ?, 'Cables', ?, (? - 0), 'NA', 'NA' FROM cables WHERE PartNo = ?";
