    ...
    try {
        ...
        if ($conn) {
            ...
            if ($statement) {
                ...
                if ($result) {
                    $response .= " and Quantity has been updated to $quantity for Part No: $partNo";
                } else {
...
if ($flag) {
    try {
        ...
        if ($conn) {
            $updateQuery = "UPDATE reorder SET Received = Received + ?, BackOrder = ?, Status = ?, lastupdated = ? WHERE PartNo = ?";
            $statement = mysqli_prepare($conn, $updateQuery);
