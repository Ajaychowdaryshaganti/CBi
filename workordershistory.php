    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            echo "<tr style='background-color: $bgcolor; color: $color;'>
                    ...
                    <td>{$row['fitter2']}</td>
                    <td>{$row['fitter3']}</td>
                    <td ><button id='fill' onclick='restore({$row['jobid']})'>Restore</button></td>
                </tr>";
        }
...
function restore(job) {
    ...
    var confirmRestore = confirm("Are you sure you want to restore this work order?");
    
    if (confirmRestore) {
        // Make an AJAX request to restorewo.php
        $.ajax({
            type: "POST",
            url: "worestore.php",
            data: { job: job }, // Pass the job parameter to the server
            success: function(response) {
                ...
                if (response.status === 'success') {
                    ...
                    console.error("Error restoring work order:", response.message);
                    alert('Error in restoring Job: ' + response.message);
                }
            },
