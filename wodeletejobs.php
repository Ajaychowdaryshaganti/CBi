...
try {
    if ($conn) {
        foreach ($jobIDs as $jobID) {
            ...
            $deleteQuery = "SELECT * FROM workorders WHERE jobid = ?";
            ...
            $deleteMainQuery = "DELETE FROM workorders WHERE jobid = ?";
            ...
            if (mysqli_stmt_execute($deleteMainStatement)) {
                ...
                if ($insertHistoryStatement) {
                    $fieldname = "work order"; // Field name for the deleted job ID
