...
while ($record = mysqli_fetch_assoc($reorderResult)) {
  ...
  echo "<tr class=\"$statusClass\">
    ...
    <td>
        <button id=\"no-fill\" onclick=\"fullOrderRecieved1('{$PNo1[$i1]}',$BackOrder1[$i1])\">Fully Received</button>
...
while ($record = mysqli_fetch_assoc($reorderResult)) {
  ...
  echo "<tr class=\"$statusClass\">
    ...
    <td>{$record['BackOrder']}</td>
    <td>
        <button id=\"no-fill\" onclick=\"fullOrderRecieved('{$PNo2[$i2]}',$BackOrder2[$i2])\">Fully Received</button>
    </td><td>{$record['Status']}</td>
    <td>
...
while ($record = mysqli_fetch_assoc($reorderResult)) {
  ...
  echo "<tr class=\"$statusClass\">
    ...
    <td>{$record['BackOrder']}</td>
    <td>
        <button id=\"no-fill\" onclick=\"fullOrderRecieved3('{$PNo3[$i3]}',$BackOrder3[$i3])\">Fully Received</button>
    </td><td>{$record['Status']}</td>
    <td>
...
function promptUser1(partNo1, oldQuantity1) {
    ...
    var xhr1 = new XMLHttpRequest();
    xhr1.open("POST", "partialOrderRecieved1.php", true);
    xhr1.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
    xhr1.onreadystatechange = function() {
        if (xhr1.readyState === 4 && xhr1.status === 200) {
            // Process the response from the PHP page
