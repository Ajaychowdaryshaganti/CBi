...
if ($conn) {
    try {
        ...
        if ($conn) { 
            ...
            $updateQuery = "UPDATE reorderconsumables SET Received = Received + ? , BackOrder = ?,Status=?, lastupdated = ? WHERE PartNo = ?";
...
if ($newbackorder<=0) {
    try {
        if ($conn) {
            // Prepare the SQL statement with placeholders
            $InsertQuery = "INSERT INTO orderhistory(BinLocation, PartName, PartNo, Category, Available, Ordered, PurchaseOrder, LastUpdated, Comments) select BinLocation, PartName, PartNo, Category,0, Received, PurchaseOrder, LastUpdated, Comments from reorderconsumables WHERE PartNo = ? ";
...
if ($newbackorder<=0) {
    try {
        if ($conn) {
            // Prepare the SQL statement with placeholders
            $deleteQuery = "DELETE FROM reorderconsumables WHERE PartNo = ?";
